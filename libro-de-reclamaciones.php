<?php
/**
 * Libro de Reclamaciones Virtual - Parkeat
...
define('DOMINIO_PERMITIDO', 'parkeatcar.com');
define('EMPRESA_RAZON_SOCIAL', 'PARKEAT S.A.C.S.');
define('EMPRESA_RUC', '20615720349');
define('EMPRESA_DIRECCION', 'Av. República de Chile Nro. 324 Int. 601 Urb. Santa Beatriz (Edificio Polaris), Lima - Lima - Jesús María');
define('EMPRESA_EMAIL', 'info@parkeatcar.com');
...
if (!defined('SMTP_FROM_NAME'))      define('SMTP_FROM_NAME', 'Parkeat');
if (!defined('EMPRESA_NOTIF_EMAIL')) define('EMPRESA_NOTIF_EMAIL', 'reclamaciones@parkeatcar.com');
...
function generarPDFReclamo(array $d): string {
    ...
    // Cabecera
    $pdf->SetFillColor(30, 64, 175);  // blue #1E40AF
    ...
    if ($success) {
        ...
        <div style='font-family:Arial,sans-serif;color:#333;max-width:600px;margin:0 auto;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;'>
            <div style='background:#1E40AF;padding:25px;text-align:center;'>
                <h2 style='margin:0;font-size:22px;color:#FFFFFF;'>HOJA DE RECLAMACIÓN VIRTUAL</h2>
                <p style='margin:5px 0 0;color:#BFDBFE;font-size:14px;font-weight:bold;'>Código: " . esc($generatedCode) . "</p>
            </div>
            <div style='padding:25px;line-height:1.6;'>
                ...
                <h3 style='color:#1E40AF;'>Detalle de la Reclamación</h3>
        ...
        $errCliente = '';
        enviarCorreoSMTP($email, 'Cargo de Hoja de Reclamación N° ' . $generatedCode . ' — Parkeat', $emailBody, $pdfB64, $pdfName, $errCliente);
        ...
        <div style='font-family:Arial,sans-serif;color:#333;max-width:600px;margin:0 auto;border:1px solid #e0e0e0;border-radius:8px;padding:25px;'>
            ...
            <h3 style='border-bottom:2px solid #eee;padding-bottom:5px;color:#1E40AF;'>1. Reclamante</h3>
            ...
            <h3 style='border-bottom:2px solid #eee;padding-bottom:5px;color:#1E40AF;'>2. Bien Contratado</h3>
            <p><strong>Tipo:</strong> " . esc(ucfirst($bien_tipo)) . " — S/. " . esc($monto) . "<br><strong>Descripción:</strong> " . esc($bien_desc) . "</p>
            <h3 style='border-bottom:2px solid #eee;padding-bottom:5px;color:#1E40AF;'>3. Reclamo</h3>
            ...
            <div style='background:#f7f7f7;padding:12px;border-left:4px solid #3B82F6;'><strong>Pedido:</strong><br>" . nl2br(esc($pedido)) . "</div>
        </div>";
...
<head>
    ...
    <title>Libro de Reclamaciones - Parkeat</title>
    <meta name="description" content="Libro de Reclamaciones Virtual de Parkeat. Presenta tus quejas o reclamos conforme al Código de Protección al Consumidor en Perú.">
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        navy: { deep: '#1E40AF', mid: '#1D4ED8', light: '#3B82F6' },
                        brand: { DEFAULT: '#3B82F6', light: '#BFDBFE', pale: '#EFF6FF' },
                    },
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .form-input::placeholder { color: rgba(30,64,175,0.3); }
        .form-input:focus { border-color: #3B82F6; box-shadow: 0 0 0 2px rgba(59,130,246,0.18); }
        ...
        .btn-sec:hover { background: rgba(30,64,175,0.06); transform: translateY(-1px); }
        ...
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(30,64,175,0.28); }
    ...
    <header class="no-print w-full border-b border-navy-deep/10 bg-white shadow-sm sticky top-0 z-50">
        <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
            <a href="index.html" class="flex items-center gap-3">
                <img src="PARKEAT.jpg" alt="Parkeat" class="h-10 object-contain rounded-lg">
            </a>
                    ...
                    <form method="POST" action="libro-de-reclamaciones.php" class="space-y-6">
                        ...
                        <div class="border-b border-navy-deep/10 pb-5">
                            ...
                            <div class="mt-4 bg-navy-deep/5 p-4 rounded-xl border border-navy-deep/5">
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" id="menor_edad" name="menor_edad" class="w-4 h-4 rounded accent-blue-600 flex-shrink-0 cursor-pointer" onclick="toggleApoderado()" <?= isset($_POST['menor_edad']) ? 'checked' : '' ?>>
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-6 mt-2">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" name="bien_tipo" value="producto" required class="w-4 h-4 accent-blue-600" <?= !isset($_POST['bien_tipo']) || $_POST['bien_tipo']=='producto' ? 'checked':'' ?>>
                                            <span class="text-navy-deep/80 text-sm">Producto</span>
                                        </label>
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" name="bien_tipo" value="servicio" class="w-4 h-4 accent-blue-600" <?= isset($_POST['bien_tipo']) && $_POST['bien_tipo']=='servicio' ? 'checked':'' ?>>
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                                        <label class="flex items-start gap-2.5 p-3 rounded-xl bg-navy-deep/5 border border-navy-deep/5 cursor-pointer hover:bg-brand-pale hover:border-brand/20 transition-colors">
                                            <input type="radio" name="reclamo_tipo" value="reclamo" required class="w-4 h-4 mt-0.5 accent-blue-600" <?= !isset($_POST['reclamo_tipo']) || $_POST['reclamo_tipo']=='reclamo' ? 'checked':'' ?>>
                                            <div>
                                        ...
                                        <label class="flex items-start gap-2.5 p-3 rounded-xl bg-navy-deep/5 border border-navy-deep/5 cursor-pointer hover:bg-brand-pale hover:border-brand/20 transition-colors">
                                            <input type="radio" name="reclamo_tipo" value="queja" class="w-4 h-4 mt-0.5 accent-blue-600" <?= isset($_POST['reclamo_tipo']) && $_POST['reclamo_tipo']=='queja' ? 'checked':'' ?>>
                                            <div>
                        ...
                        <div class="pt-4 border-t border-navy-deep/10 space-y-3">
                            <label class="flex items-start gap-3 cursor-pointer">
                                <input type="checkbox" required class="w-4 h-4 mt-0.5 rounded accent-blue-600 flex-shrink-0 cursor-pointer">
            ...
            <div class="space-y-6">
                ...
                <div class="glass rounded-3xl p-6 shadow-md">
                    ...
                    <div class="relative rounded-2xl overflow-hidden border border-navy-deep/10 bg-navy-deep/5 group">
                        <img src="Libro-reclamaciones/PARKEAT.png" alt="Aviso Virtual de Libro de Reclamaciones - Parkeat" class="w-full h-auto object-cover opacity-80 group-hover:opacity-100 transition-opacity">
    ...
    <div id="notice-modal" class="no-print fixed inset-0 z-[100] hidden items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
        ...
        <div class="relative z-10 max-w-lg w-full bg-white rounded-3xl p-4 shadow-2xl flex flex-col items-center">
            ...
            <div class="w-full overflow-y-auto max-h-[80vh] border border-gray-200 rounded-2xl">
                <img src="Libro-reclamaciones/PARKEAT.png" alt="Aviso Virtual Oficial - Parkeat" class="w-full h-auto">
            </div>
